            ...
            case PlayerState.Moving:
                ...
                if (enemy != null)
                {
                    ...
                    if (!isFleeing)
                    {
                        ...
                        if (inRange)
                        {
                            // 공격 우선도 높이기 -> 0608 코드 수정
                            if (!enemyIsBlocking && core.CanAttack())
                            {
                                // 공격형: 공격 시도 확률 더 높여서 90%
                                if (Random.value < 0.90f) // 확률 추가
                                {
                                    LookAtEnemy();
                                    core.Attack();
                                }
                                if (agent != null)
                                    agent?.ResetPath();
                                // 공격 후 후퇴 확률을 60%로 설정해 전투 유지성↑
                                if (Random.value < 0.60f)
                                    Flee();
                                return;
                            }

                            // 공격 불가 시 20% 확률로 방어 시도
                            if (!core.CanAttack() && core.CanDefence() && Random.value < 0.20f)
                            {
                                LookAtEnemy();
                                core.Defence();
                                if (agent != null)
                            ...
                            // 공격 불가 시 회피 고려 -> 0608 코드 수정
                            if ((!core.CanAttack() || Random.value < 0.2f) && core.CanDodge()) // 20%로 설정
                            {
                                core.Dodge();
                                if (agent != null)
                                    agent?.ResetPath();
